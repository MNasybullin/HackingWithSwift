...
//  Project25
//
//  Created by Stomach Diego on 10/15/20.
...
class ViewController: UICollectionViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, MCSessionDelegate, MCBrowserViewControllerDelegate {
    ...
    @objc func showPeers() {
        let message: String!
        if (mcSession != nil) {
            message = "Devices in session : \(mcSession!.connectedPeers)"
        } else {
            message = "Session not found."
        }
        let ac = UIAlertController(title: "Session info", message: message, preferredStyle: .alert)
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        ...
        case .notConnected:
            ...
            let ac = UIAlertController(title: "Session info", message: "\(peerID.displayName) has disconnected.", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "OK", style: .default))
            present(ac, animated: true)
        @unknown default:
